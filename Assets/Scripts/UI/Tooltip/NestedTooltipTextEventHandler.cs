...
public class NestedTooltipTextEventHandler : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
{
    ...
    public class LinkUnhoverEvent : UnityEvent<string, string, int> { }
    ...
    /// <summary>
    /// Event delegate triggered when pointer exits a previously hovered link.
    /// </summary>
    public LinkUnhoverEvent onLinkUnhover
    {
        get { return m_OnLinkUnhover; }
        set { m_OnLinkUnhover = value; }
    }
    [SerializeField]
    private LinkUnhoverEvent m_OnLinkUnhover = new LinkUnhoverEvent();
    ...
    void LateUpdate()
    {
        if (TMP_TextUtilities.IsIntersectingRectTransform(m_TextComponent.rectTransform, Input.mousePosition, m_Camera))
        {
            ...
            if (linkIndex == -1 && m_selectedLink != -1)
            {
                ...
                SendOnLinkUnhover(oldInfo.GetLinkID(), oldInfo.GetLinkText(), m_selectedLink);
                m_selectedLink = -1;
            }
            ...
            if (linkIndex != -1 && linkIndex != m_selectedLink)
            {
                ...
                if (m_selectedLink != -1)
                {
                    ...
                    SendOnLinkUnhover(prev.GetLinkID(), prev.GetLinkText(), m_selectedLink);
                }
                m_selectedLink = linkIndex;
        ...
        else
        {
            ...
            if (m_selectedLink != -1)
            {
                ...
                SendOnLinkUnhover(exitInfo.GetLinkID(), exitInfo.GetLinkText(), m_selectedLink);
            }
    ...
    /// <summary>
    /// Invoke when pointer exits a link.
    /// </summary>
    private void SendOnLinkUnhover(string linkID, string linkText, int linkIndex)
    {
        if (onLinkUnhover != null)
            onLinkUnhover.Invoke(linkID, linkText, linkIndex);
    }
